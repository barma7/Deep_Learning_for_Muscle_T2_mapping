function [T2fat, T2water, B1fit, tElapsed] = FatCal_dictionary_matching_dot(LUT, Dict, Data)

% Function to estimate T2fat, T2w and B1 from MESE data with dictonary
...
%------------------ OUTPUT PARAMETERS -------------------------------------
% T2fat .........Array with fitted  T2 fat [m x 1]
% T2water.........Array with fitted T2 water [m x 1]
...
T2fat = zeros(Nb_tests, 1);
T2water = zeros(Nb_tests, 1);
...
for b = 1:Nb_blocks
    if (b == Nb_blocks && stop_indx>Nb_tests)
        ...
        for j=1:length(maxDot_array) 
            ...
            T2fat(indx) = LUT(Mtch_indx,1);
            T2water(indx) = LUT(Mtch_indx,2);
