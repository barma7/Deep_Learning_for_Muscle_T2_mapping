...
dataset_folder = join(homeDir, "DATA", "PHILIPS")
root_name = "AM17"
subjects = ["05", "08", "09", "14", "16"]

dcm_names = ["IM_0005", "IM_0009", "IM_0013"]
nifti_names = ["201_qt2_-mt-_s1.nii.gz", "202_qt2_-mt-_s2.nii.gz", "201_qt2_-mt-_s1.nii.gz"]

ffmap_names = ["ff_s1.nii.gz", "ff_s2.nii.gz", "ff_s3.nii.gz"]
t2map_names = ["t2_s1.nii.gz", "t2_s2.nii.gz", "t2_s3.nii.gz"]
b1map_names = ["b1_s1.nii.gz", "b1_s2.nii.gz", "b1_s3.nii.gz"]
tse_names = ["TSE_s1.nii.gz", "TSE_s2.nii.gz", "TSE_s3.nii.gz"]

sv_root = "MAPS/DL/FatNet-snr-1e3-1e4"
sv_fldr = "MuscleNet/Noise/var-1e-5-5e-7_t2w-10-110-t2f-50-200-b1-1.2"
sv_name = join(sv_root, sv_fldr)

## NN Parameters
# model3 W/ B1 estimation
wieghts_path = join(homeDir, r"DeepLearning\training\results\PHILIPS\ETL_17_ESP_7.6\MuscleNet\Noise\var-1e-5-5e-7_t2w-10-110-t2f-50-200-b1-1.2")

# FatNet
FatModel_weights =join(homeDir, r"DeepLearning\training\results\PHILIPS\ETL_17_ESP_7.6\FatNet\Noise\fixed-FF\snr-e3-e4-t2w-10-110-t2f-50-200")
...
MuscleNet.load_weights(join(wieghts_path, 'weights', 'MODEL.500.hdf5'))
...
for sub in subjects:
    ...
    for stack in range(0, len(nifti_names)):
        print('processing stack ', stack)

        nn_svfldr = join(dataset_folder, root_name + sub, "nifti", sv_root)
        SvFldr = join(dataset_folder, root_name + sub, "nifti", sv_name)
        Path(SvFldr).mkdir(parents=True, exist_ok=True)

        #READ DATA
        stack_nifti = nr.load(join(dataset_folder, root_name + sub, "nifti", tse_names[stack]))
        # plot a data with matplotlib: dim 2 = 12, dim 3 = 0
        # plt.imshow(stack_nifti.volume[:, :, 12, 0], cmap='gray'), plt.show()
        ...
        if not os.path.exists(join(nn_svfldr, "Fat_calibration.csv")):
            ...
            fat_nifti = nr.load(join(dataset_folder, root_name + sub, "nifti", tse_names[1]))
            fat_mask = nr.load(join(dataset_folder, root_name + sub, "nifti", "fat_mask1.nii.gz"))
            fat_mask = fat_mask[..., ::-1]
            ...
            sub_fat_data = fat_Data[:, :, int(np.round(nb_slices/2)) - 2: int(np.round(nb_slices/2)) + 2, :]
            
            # flat sub_fat_data
            sub_fat_data_flat = np.reshape(sub_fat_data, (nb_row*nb_col*sub_fat_data.shape[2], nb_echoes))
            fat_mask_flat = fat_mask.volume.flatten()

            fat_data = sub_fat_data_flat[fat_mask_flat > 0]
            ...
            t2_water_scatter = pred[:,0].reshape(-1, 1)
            t2_fat_scatter = pred[:,1].reshape(-1, 1)
            ...
            t2_fat_mean = np.mean(t2_fat_scatter)
            t2_water_mean = np.mean(t2_water_scatter)

            t2_fat_geom = gmean(t2_fat_scatter)
            t2_water_geom = gmean(t2_water_scatter)
        ...
        FFmap_mv = dm.MedicalVolume(FFmap, stack_nifti.affine)
